...
# Drosophila Spike-in counts for Normalization
...
echo -e "Name\tDm_Reads\tDm_Reads_MapQ\tDm_Reads_MapQ_NoDup\tMm_Reads\tMm_Reads_MapQ\tMm_Reads_MapQ_NoDup\tUnmapped" >> spike_stats.txt
...
do
	...
	$VIEW $FILE | awk -v dm=$DM_SAM -v mm=$MM_SAM '{
		if ($3 ~ "dm*") {
			print > dm;
		} else {
			print > mm;
	...
	$VIEW -H $FILE | awk -v dm=$HEADER_DM -v mm=$HEADER_MM '{
		if ($2 ~ /^SN/) {
			if ($2 ~ /dm/) {
				print > dm;
			} else {
	...
	echo -e "$FILE\t$DM\t$DM_MQ\t$DM_MQDP\t$MM\t$MM_MQ\t$MM_MQDP\t$UM" >> spike_stats.txt
